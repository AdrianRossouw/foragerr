...
| 2026-07-10 | `ceb18cd5661e5e1d4e0dc4b8749c7839227d83f1` | gitleaks 8.24.3 + .gitleaks.toml | 0 | m4-series-detail gate re-scan: 12 findings = 11 dispositioned fixtures + KA-001 (accepted) — no new findings |
| 2026-07-12 | `1950c3d7afb850477cf82e8127b721e2dea04f4e` | gitleaks 8.24.3 + .gitleaks.toml | 0 | m8-keys-opds gate re-scan (578 commits, first since m4-series-detail — covers M5–M8 merges): 24 findings = 23 dispositioned synthetic test fixtures/false-positives + KA-001 (accepted). Growth from 11→23 is entirely new synthetic fixtures added by M5/M6/M8 tests (`aBcD1234synthetic`, `CV-SECRET-KEY-abc123`, `sab-secret-key-4321`, `raw-key-abc123`) and non-secret config-line matches (`comicvine_min_interval_seconds=…`, `signature/expiry` prose) — each value inspected, none real. No new genuine findings |
| 2026-07-12 | `d075fb12b51b928631caabc1bdd56ddd3857b66c` | gitleaks 8.24.3 + .gitleaks.toml | 0 | m8-rate-audit gate re-scan (745 commits, pre-merge HEAD incl. gate-fix commit): 24 findings = identical dispositioned set to the m8-keys-opds scan above (23 synthetic fixtures/false-positives + KA-001). This change added only stdlib auth code + tests; its new fixtures (`bob surface=api_key…` audit-forge string) are not secret-shaped and raise no finding. No new genuine findings |
| 2026-07-12 | `1d76c81d8cac0ea969912a8840c805b22db43c2b` | gitleaks 8.24.3 + .gitleaks.toml | 0 | m8-rate-audit-followups (v0.9.1) gate re-scan (750 commits): 24 findings = identical dispositioned set (23 synthetic fixtures/false-positives + KA-001). The patch touched only the login screen, auth helpers, and docs; no secret-shaped strings added. No new genuine findings |
| 2026-07-28 | `2dc04c78aa05a6695f05ee06c942194d06e9d332` | gitleaks 8.24.3 + .gitleaks.toml | 0 | m11-source-import-trust gate re-scan (722+ commits, pre-merge HEAD incl. simplify pass): 26 findings = 24 previously dispositioned (23 synthetic fixtures/false-positives + KA-001, accepted) + 2 new instances of the SAME dispositioned synthetic `aBcD1234synthetic` fixture in files this change added/moved (`test_review_freshness.py`, `sources_support.py`). No new genuine findings |
| 2026-07-28 | `f53151abfc2c052f16f0916499357897dfe33d4a` | gitleaks 8.24.3 + .gitleaks.toml | 0 | m11-review-experience gate re-scan (pre-merge HEAD): 26 findings — the identical dispositioned set as the m11-source-import-trust row above (24 prior + 2 instances of the same synthetic fixture); the synthetic-title fixture scrub introduced no secret-shaped strings. No new genuine findings |
| 2026-07-28 | `22357cbedfaad293b05ee24639713b5a35538cf9` | gitleaks 8.24.3 + .gitleaks.toml | 0 | m11-cv-budget gate re-scan (pre-merge HEAD): 27 findings — the prior dispositioned set + one more instance of the dispositioned synthetic `CV-SECRET-KEY-abc123` fixture in the new enrichment test file. No new genuine findings |
