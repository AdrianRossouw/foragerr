...
| FRG-SRCH-015 | Per-indexer interactive time budget with partial results | srch | approved | M11 |
...
| FRG-DL-015 | Import-visibility stall escalation | dl | approved | M11 |
...
| FRG-PP-021 | Provenance-authoritative series resolution | pp | implemented | M11 |
| FRG-PP-022 | Ordinal-fallback issue resolution under a known series | pp | implemented | M11 |
...
| FRG-UI-033 | Actionable UI-language guidance with settings links | ui | implemented | M9 |
| FRG-UI-034 | Inline root-folder creation in the add dialog | ui | implemented | M9 |
...
| FRG-UI-041 | Per-indexer search outcomes | ui | approved | M11 |
...
| FRG-OPDS-020 | Per-issue cover on issue entries; series cover on the shelf | opds | implemented | M9 |
...
| FRG-SCHED-012 | First-indexer sweep and next-search visibility | sched | approved | M11 |
...
| FRG-DEP-017 | Documented deployment security posture | dep | implemented | M10 |
...
| FRG-SEC-006 | HTTP security response headers | sec | implemented | M10 |
| FRG-SEC-007 | Opt-in trusted-proxy handling | sec | implemented | M10 |
| FRG-SEC-008 | Unauthenticated disclosure and error hygiene | sec | implemented | M10 |
...
| FRG-NFR-016 | Library-import group failures logged at WARNING with reasons | nfr | implemented | M9 |
