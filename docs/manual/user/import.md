...
event, so a mistaken upgrade or deletion is recoverable. User-initiated
deletes (a single issue's file from the series detail screen, or a series
deleted with its files) route through the same bin. A housekeeping task
