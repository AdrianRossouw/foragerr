...
| `indexer_search_time_budget_seconds` | `FORAGERR_INDEXER_SEARCH_TIME_BUDGET_SECONDS` | `20.0` | Per-indexer time budget for an **interactive** search: when it lapses, finished indexers' results return immediately and the slow indexer is reported as timed out on the results (never a whole-request failure). Clamped to `5..60`, and clamped further to `listener_request_timeout_seconds` minus 5 s — so with the default 30 s request guard the effective maximum is 25 s, and lowering the guard tightens the budget with it. A real search walks a multi-rung query ladder at the per-indexer politeness interval, so one indexer structurally needs ~15 s even when idle — budgets below that time out honest indexers. Interactive requests take precedence at the politeness gate, so background sweeps never starve a search you are watching. Scheduled background searches are never budgeted — politeness outranks latency when nobody is waiting. |
| `import_stall_threshold_cycles` | `FORAGERR_IMPORT_STALL_THRESHOLD_CYCLES` | `5` | Consecutive import cycles that could not SEE a completed download's files before application health degrades with mount/path guidance (the completed-but-invisible stall). Counts only genuine visibility failures: a missing/unreadable path, or a client path with no remote path mapping to translate it. A path foragerr can read that simply holds nothing importable (an unrepaired `.rar`/`.par2` set, say) is a release problem, not a mount problem — it blocks per cycle as usual and never degrades health. Minimum 2. The queue row keeps its normal per-cycle state either way. |
...
| `comicvine_error_retry_spacing_seconds` | `FORAGERR_COMICVINE_ERROR_RETRY_SPACING_SECONDS` | `43200` | Minimum spacing before the *scheduled* enrichment path re-attempts an entitlement whose ComicVine consultation errored (a 500, a transport failure). Keeps a persistently failing row from burning budget every night. The default (12 h) is deliberately **below** `source_sync_interval_seconds` (24 h): a spacing equal to the sync interval loses the race with it, so an errored row would be skipped for a whole extra day and retried only every *other* night. Keep it under your sync interval if you change either. Operator actions (restore, the review row's search, the bulk recompute) are never spaced; manual "Sync now" runs the scheduled path and *does* space. |
...
| `comicvine_ignored_publishers` | `FORAGERR_COMICVINE_IGNORED_PUBLISHERS` | `Panini*, Urban Comics, Planeta DeAgostini, Editorial Televisa, Ediciones Zinco` | Comma-separated publisher names hidden from Add New series-search results — foreign-market reprint imprints that otherwise outrank the original-language volume. Case-insensitive; an entry ending in `*` matches as a substring (`Panini*` covers Panini Verlag/España/France), any other entry matches exactly. **Editable in the UI (Settings → General).** The curated default seeds **fresh installs only** — an existing config keeps its stored value (set it to an empty string to hide nothing); upgraders who want the default opt in by editing this setting or the UI field. (One edge: a pre-v0.2 config so old it never carried the line at all acquires the default when its file is migrated.) The list also filters library-import and store-source automatic match proposals, not just Add New. Hidden results are counted and stay recoverable per-search in Add New (see `../user/search.md`). |
| `non_comic_publishers` | `FORAGERR_NON_COMIC_PUBLISHERS` | curated non-comic publisher set (see below) | Comma-separated publisher names classified as **Other** (not a comic) at store-source sync, library-wide across every connected source (`FRG-SRC-012`). Matching is case-insensitive and ignores punctuation and spacing differences (`Modiphius Entertainment.` and `modiphius  entertainment` are one name); only a **trailing** `*` widens an entry to match any publisher name *containing* it (`Paizo*` covers "Paizo Inc." and "Paizo Publishing"), and every other entry has to match the whole name. A `*` anywhere but the end is treated as part of the name, so it matches nothing. Entries are separated by commas, which means a publisher name cannot itself contain one — write `Monte Cook Games*` rather than `Monte Cook Games, LLC` (the UI refuses a comma in a single entry for the same reason). **Editable in the UI (Settings → General), beside the ComicVine ignore list.** Ships with a curated, fully removable default so a fresh install files common non-comic bundle content with no setup: role-playing/game houses (Paizo, Pelgrane Press, Green Ronin, Kobold Press, Free League, Modiphius, Chaosium, Cubicle 7, Evil Hat, Monte Cook Games, Goodman Games, Onyx Path, Steve Jackson Games, R. Talsorian, Renegade Game Studios) and tech/textbook houses (O'Reilly, No Starch, Manning, Packt, Pragmatic Bookshelf, Apress, Wiley, Addison-Wesley, Pearson, CRC Press, Mercury Learning) — each shipped as a `*` entry, since stores report the same house with and without its corporate suffix ("Monte Cook Games" and "Monte Cook Games, LLC"). Remove any default whose books you do collect — every entry, default or added, is removable. The default seeds **fresh installs only**; an existing value — including one carried forward from an earlier per-source publisher-rules list on upgrade — is left untouched. A change only reclassifies items still awaiting review (`new`); anything already matched or ignored never moves, and it takes effect on the **next sync**, not retroactively. Reclassified items are never dropped — they stay visible under **Show other items**, same as any other non-comic item (see `../user/sources.md` → "What sync does"). If the environment variable is set, the Settings field shows read-only with a note that it's environment-managed, the same treatment `comicvine_api_key` gets (see "Setting the ComicVine key" below). |
...
| `pull_backfill_weeks` | `FORAGERR_PULL_BACKFILL_WEEKS` | `4` | Extra previous release weeks fetched when the pull store is empty (fresh install) so the Calendar starts with recent history (`FRG-PULL-010`). One-shot; `0` disables; capped at 12. |
...
| `opds_hide_fileless_series` | `FORAGERR_OPDS_HIDE_FILELESS_SERIES` | `false` | Opt-in: omit series with no downloadable files from the OPDS **All Series** feed, so a reader browses only shelves that contain something to read (a hidden series appears the moment its first file imports, and stays findable via OPDS search). Off by default — the shelf mirrors the full library, wanted series included. |
...
- **Approaching the budget ceiling** — a path crossed the warning fraction of
  ...
  work defers before interactive requests are ever affected). It clears as the
  rolling hour drains the window.

A **paused background lane is not a health warning.** Background work stopping at
its `comicvine_batch_budget_share` is the reserve doing its job — your own
searches, adds and review actions keep working, and background work resumes on
its own — so it does not appear in Health. It is shown on the meter instead,
with its own resume countdown, because the numbers are still worth seeing.

The **budget meter** (Settings → General, beside the ComicVine key) shows all of
it continuously: per-path usage, the batch share and where it ends, and any
resume countdown for either the path or the background lane. The Sources review
screen shows a compact indicator only for a path near its ceiling — the meter is
where you go to look, the indicator is for when you were not looking. ComicVine's
API exposes no usage counters of its own (verified), so this local meter is the
...
- Which items a sync files as **Other** rather than a comic is controlled by
  **`non_comic_publishers`** above — a library-wide list, not a per-source
  setting, edited in Settings → General.
