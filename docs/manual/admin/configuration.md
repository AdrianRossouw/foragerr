...
| `indexer_search_time_budget_seconds` | `FORAGERR_INDEXER_SEARCH_TIME_BUDGET_SECONDS` | `20.0` | Per-indexer time budget for an **interactive** search: when it lapses, finished indexers' results return immediately and the slow indexer is reported as timed out on the results (never a whole-request failure). Clamped to `5..60`, and clamped further to `listener_request_timeout_seconds` minus 5 s — so with the default 30 s request guard the effective maximum is 25 s, and lowering the guard tightens the budget with it. Scheduled background searches are never budgeted — politeness outranks latency when nobody is waiting. |
