...
8. Comment hygiene (FRG-PROC-023, public repository): every review gate's
   angles include a comment-hygiene pass over the diff — each comment,
   docstring, test name and fixture literal states a constraint rather than
   narration or review provenance, carries no environment-specific value
   (rig host/port, container name, local path, operator identity), and uses
   synthetic example identifiers — and `tools/comment_check.py` exits 0.
   Standard: `docs/process/code-comments.md`.
9. Registry rows flipped, matrix regenerated, change archived, branch deleted
   after merge.
10. Release record per FRG-PROC-013 (`openspec/specs/dev-process/spec.md` owns the
