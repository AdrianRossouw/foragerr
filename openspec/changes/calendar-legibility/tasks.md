...
- [ ] 1.1 Replace the Calendar's control-shaped state glyph with a status
...
- [ ] 2.1 Record the 900px crossover once in the layout token layer as the
      single value both this screen and the shell's media queries read
- [ ] 2.2 Agenda-row mode at or above the crossover: fixed-size thumbnail,
...
- [ ] 2.3 Quiet-card mode below the crossover: single column, title spans the
...
- [ ] 2.4 One action set for both modes — no affordance exists in one and not
      the other; covers/spine fallback (FRG-UI-042), debut badge, detail
      expando, not-yet-released marking all preserved in both
- [ ] 2.5 Tagged vitest (IDs in names): wide mode renders rows with an
...
- [ ] 3.1 Optimistic + busy state on the toggle: requested state rendered
      immediately, further activations suppressed until the mutation settles
- [ ] 3.2 Settle to the re-projected derived state on success (existing
...
- [ ] 3.3 Tagged vitest (ID in the name): immediate requested-state render;
...
- [ ] 4.1 Sidebar becomes an off-canvas drawer below the crossover: hidden by
...
- [ ] 4.2 Drawer keyboard behavior: focus into the drawer on open, Escape
      closes and returns focus to the toggle, nav selection navigates and
      closes
- [ ] 4.3 Tagged vitest (ID in the name): narrow render exposes the toggle and
...
- [ ] 5.1 `docs/manual/user/web-ui.md`: rewrite the **Calendar** section's
...
- [ ] 5.2 Registry rows for FRG-UI-047/048/049 flipped `proposed` →
      `implemented`; traceability regenerated and clean
- [ ] 5.3 Refresh the README tour shots if the Calendar's appearance changed in
      any embedded screenshot (FRG-PROC-017)
