...
- [ ] 1.1 Add `parse_nzb_xml` to `backend/src/foragerr/indexers/xml.py`
...
- [ ] 1.2 Switch `_validate_nzb` (`downloads/clients/sabnzbd.py`) to
      `parse_nzb_xml`; docstring updated.
- [ ] 1.3 Tests: real-shape DOCTYPE-bearing NZB fixture accepted
...
- [ ] 1.4 Baseline specs amended (sec + dl per delta files); threat model note
      (`docs/security/threat-model.md`) — RISK-024/035/037 unaffected.
- [ ] 1.5 Live UAT against the standing rig: re-grab Saga #1 release through
...
- [ ] 2.1 Sort unordered joins in `tools/trace.py` cell emission; regenerate
      `docs/traceability/matrix.md` once (one-time reorder diff).
- [ ] 2.2 Test: matrix regeneration is byte-stable across two runs
      (`@pytest.mark.req("FRG-PROC-005")`).

## 3. README sources screenshot (owner request)

- [ ] 3.1 `e2e/scripts/capture-readme-shots.ts` `sources` step: branch on
...
- [ ] 3.2 Capture against the live-connected :8793 instance; commit refreshed
      `docs/readme-assets/sources.png`.

## 4. Release (FRG-PROC-013 + merge gate)

- [ ] 4.1 CHANGELOG v0.6.3 entry + `backend/pyproject.toml` → 0.6.3.
