...
- [ ] A.1 Ordered image-member listing: given a confined archive path + `ArchiveLimits`,
...
- [ ] A.2 Safe single-member reader: byte-cap-BEFORE-read (member.file_size vs cap),
...
- [ ] A.3 Image decode/downscale/encode + first-page cover extraction (Pillow): lazy
...
- [ ] B.1 PSE stream endpoint `GET {base}/page/{issue_file_id}/{pageNumber}?width=`:
      ...
      field). [FRG-OPDS-008]
- [ ] B.2 Local cover endpoint `GET {base}/cover/{issue_file_id}` (+`?thumbnail`):
...
- [ ] B.3 Emission: declare `xmlns:pse` on the feed; extend atom `Link` with a
...
- [ ] C.1 `issue_files.page_count` nullable column + forward-only migration `0012`
...
- [ ] C.2 Lazy first-access compute + write-back + size-mismatch invalidation (a small
...
- [ ] C.3 Config keys `opds_pse_max_members`, `opds_pse_max_page_bytes`,
...
- [ ] D.1 SOUP (FRG-PROC-012): add **Pillow** to `docs/security/soup-register.md`
...
- [ ] D.2 Security (FRG-PROC-006): `docs/security/threat-model.md` COMP 3 (OPDS) gains a
      ...
      [FRG-PROC-006]
- [ ] D.3 Manual (FRG-PROC-011): `docs/manual/user/` note that PSE readers (Panels/
...
- [ ] D.4 Registry + matrix: FRG-OPDS-008..012 flip `approved → implemented`;
