...
- [x] 1.4 Bootstrap seeding: first authed boot seeds principal + OPDS
...
- [x] 2.1 `auth/sessions.py`: opaque 256-bit tokens, SHA-256 rows, two
      sliding tiers, token regeneration on login, server-side logout,
      password-change invalidation-except-acting (FRG-AUTH-004)
- [x] 2.2 Login/logout routes: form login with remember-me flag, generic
      failure responses, HttpOnly SameSite=Lax cookie, `Secure` conditional
      on transport with documented decision (FRG-AUTH-002/004)
- [x] 2.3 Session prune job on the existing scheduler (FRG-AUTH-004)

## 3. Perimeter (backend)

- [x] 3.1 Root auth dependency accepting the designated credential per
...
- [x] 3.2 CSRF stance: Origin/Referer check on unsafe methods under cookie
      auth (FRG-SEC-005)
- [x] 3.3 WebSocket handshake: same auth dependency + Origin allowlist,
      refusal pre-upgrade (FRG-AUTH-010, FRG-SEC-005)

## 4. Frontend

- [x] 4.1 Login screen (username/password/remember-me, tokens-compliant,
      minimal) + logout control
- [x] 4.2 API client 401 interception → login redirect with return path;
      WS client re-auth handling

## 5. Tests (tagged per requirement)

- [x] 5.1 Route-inventory test: every registered route exempt-listed or
      refuses bare requests; new-router probe covered by construction
      (FRG-AUTH-010)
- [x] 5.2 Password/KDF tests: hash-only at rest, verify/reject,
      constant-time path, no credential material in logs (FRG-AUTH-003)
- [x] 5.3 Session tests: cookie attributes, tier sliding expiry, fixation
      regeneration, logout replay 401, password-change invalidation, prune
      (FRG-AUTH-004)
- [x] 5.4 Bootstrap tests: fail-fast without env pair, seed, re-seed
      recovery + session invalidation (FRG-AUTH-002)
- [x] 5.5 CSRF + WS Origin tests: foreign-Origin unsafe method rejected,
      API-key surface immune, cross-origin WS refused pre-upgrade,
      configurable proxy origin (FRG-SEC-005)
- [x] 5.6 Frontend tests: login flow, 401 redirect, logout (IDs in names)
- [x] 5.7 e2e: negative paths per surface (UI/API/OPDS/WS refused bare),
...
- [x] 6.1 `docs/security/`: STRIDE session/cookie/CSRF rows; RISK-020
      Accept → Mitigated; RISK-022/G-5 closed; secrets docs gain
      "environment trust class" section (FRG-PROC-006)
- [x] 6.2 `docs/manual/`: login, remember-me, env bootstrap + recovery,
      upgrade/BREAKING block; README auth-posture labelling if touched
      (FRG-PROC-011)
- [x] 6.3 CHANGELOG v0.7.0 entry (BREAKING upgrade block) + backend
      pyproject bump (FRG-PROC-013)
- [x] 6.4 Verify `soup_check` 0 (no dependency changes) and `trace.py` 0
