...
- [ ] A.1 `ComicVineClient.suggest_series(term)`: a single-page fetch (offset 0,
...
- [ ] A.2 `GET /api/v1/series/lookup/suggest?term=` route (registered BEFORE
...
- [ ] B.1 Suggest API hook + fetcher wiring: term-keyed React Query for the
...
- [ ] B.2 Add Series autosuggest dropdown: renders bounded suggest candidates,
...
- [ ] B.3 Header quick-search component in `AppShell`: client-only fuzzy match
...
- [ ] B.4 Always-present "Search ComicVine for '<term>'…" fall-through row →
...
- [ ] C.1 Extend the Playwright/e2e harness on the assembled slice: seed a
...
- [ ] D.1 Manual (FRG-PROC-011): `docs/manual/web-ui.md` — Add Series section
...
- [ ] D.2 Security (FRG-PROC-006): `docs/security/` delta — STRIDE note on the new
