...
- [x] 1.1 Add `EXT | Companion browser extension` to the AREA table in
  `docs/process/commit-standard.md`.
- [x] 1.2 Allocate FRG-EXT-001..003 in
  `docs/traceability/requirements-registry.md` (status `proposed`, milestone 0.9.x).
  No `SRC` allocation — no backend change.
- [x] 1.3 Draft the threat-model additions (T-EXT-1 clipboard residual, T-EXT-2 build
...
- [x] 1.4 Add risk-register row RISK-050 (self-distributed build integrity) to
...
- [x] 1.5 Record owner approval in proposal.md `## Approval` before implementation
  (FRG-PROC-009 gate).

## 2. Extension: shared source (FRG-EXT-001, FRG-EXT-002)

- [x] 2.1 Create `extension/` at repo root: `src/popup.html`, `src/popup.js`,
  `src/background.js` (no-state MV3 worker), shared `manifest.base.json`. Plain
  JS/HTML/CSS, no framework, no `storage`.
- [x] 2.2 Implement click-time cookie read
...
- [x] 2.3 "Log in to Humble first" handling when no `_simpleauth_sess` cookie exists;
  optional read-only expiry-timestamp hint in the popup.
- [x] 2.4 Manifest: exactly `["cookies","clipboardWrite"]` + the single Humble host;
...
- [x] 3.1 `extension/build.mjs` (Node stdlib only): emit `dist/chrome/` (MV3
...
- [x] 3.2 Verify no new SOUP entry is required (`tools/soup_check.py` stays green);
  document the dependency-free build in `extension/README.md`.
- [x] 3.3 Firefox unlisted AMO-signing steps and Chrome developer-mode load documented
  in `extension/README.md`; note the byte-compare verification path.

## 4. Extension tests (FRG-EXT-001..003)

- [x] 4.1 Unit tests (Node's built-in `node:test`, IDs in test names) over the pure copy
  logic with injected cookie/clipboard fakes: cookie read only on the click path;
  clipboard written with the cookie value; cookie not retained post-copy;
  no-Humble-session branch copies nothing.
- [x] 4.2 Manifest assertion test: both built manifests declare exactly
  `["cookies","clipboardWrite"]` + the single Humble host, no content scripts / broad
  permissions / extra host (FRG-EXT-002).
- [x] 4.3 Source-scan test: no `fetch`/`XMLHttpRequest`/WebSocket in the extension
  source (no-network invariant, FRG-EXT-002).
- [x] 4.4 Deterministic-build test: two runs produce byte-identical zips (FRG-EXT-003).

## 5. Docs

- [x] 5.1 `docs/manual/`: Sources chapter — add "Copy the cookie with the browser
...
- [x] 5.2 Replace the Sources connect-card "extension coming soon" helper text with a
  link to the new manual section (frontend).
- [x] 5.3 README: one feature line for the companion extension.
- [x] 5.4 Regenerate the traceability matrix; flip FRG-EXT-001..003 to `implemented`;
