...
- [ ] 1.1 Event writers: `EVENT_GRABBED` at grab time (search_ops/grab.py,
...
- [ ] 1.2 `record_event_deduped` in importer/history.py (newest blocked/failed
...
- [ ] 1.3 `GET /api/v1/history` (queue pattern: paginate + batch series/issue
...
- [ ] 2.1 `GET /api/v1/wanted/missing` over repo.wanted_issues (paged, nested
      series). Tests: derived semantics (import removes, delete returns), no
      stored status. [FRG-API-012]
- [ ] 2.2 `GET /api/v1/blocklist` (paged, nested series/issue) +
...
- [ ] 3.1 Emit CommandStatusChanged on the started claim (commands/service.py
...
- [ ] 4.1 `/opds/recent` acquisition feed (added_at desc, shared paging
      helpers) + root entry. Tests: newest-first by import time, full
      acquisition entries, page clamp. [FRG-OPDS-013]
- [ ] 4.2 OpenSearch (a): root rel=search link, descriptor document, /opds/
...
- [ ] 5.1 `DELETE /api/v1/issuefile/{id}` router → delete_issue_file (404 map;
      source=manual for the history event). Tests. [FRG-API-003, FRG-UI-004]
- [ ] 5.2 delete_series(delete_files=True): files through recycle routing
...
- [ ] 6.2 Wanted screen (top-level nav): rows w/ series links + per-issue
      automatic/interactive search, Search All (backlog-search command +
      status), explicit empty state. [FRG-UI-011]
- [ ] 6.3 Blocklist screen (Activity group): reason cells verbatim, per-item +
      bulk remove w/ partial-failure reporting. [FRG-UI-017]
- [ ] 6.4 SeriesDetail: per-issue delete-file action (confirmation names
      bin-vs-permanent), delete-files checkbox wired for real; invalidations
      (series, wanted, history). [FRG-UI-004]
- [ ] 6.5 WS bridge branches + queryKeys for history/wanted/blocklist where
...
- [ ] 7.3 Registry flips (API-011, API-012, UI-010, UI-011, UI-017, SCHED-010,
      OPDS-007, OPDS-013) + matrix regen + soup 0. [FRG-PROC-004, FRG-PROC-005]
- [ ] 7.4 e2e: extend spine or add scenario — history shows the spine's
      grab→import; wanted lists a missing issue; OPDS recent serves the
      imported file. [FRG-PROC-010]
