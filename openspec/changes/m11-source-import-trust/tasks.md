...
- [ ] 1.1 Allocate FRG-PP-021, FRG-PP-022, FRG-IMP-026, FRG-SRC-008,
...
- [ ] 1.2 Parser filler stripping: consume anchored bare "Issue"/"Issues"
      before issue evidence so it never enters series_name; unit-safe for
      mid-title usage (FRG-IMP-026)
- [ ] 1.3 Corpus rows appended (count assert bumped): SPAWN Vol. 243 /
...
- [ ] 2.1 Split `_reconcile_base` step 2: series-only grab hint resolves
      issue within the provenance series; never falls through to
      unscoped step 3; honest series-scoped block reason (FRG-PP-021)
- [ ] 2.2 Ordinal fallback: synthetic Issue(volume_ordinal) matched via
...
- [ ] 2.3 Tests: series-only hint import (the finding-#15 filename
...
- [ ] 4.1 `add_entitlement`: pre-check cv_volume_id in library → delegate
      to match; narrow the catch-all 400 (FRG-SRC-008)
- [ ] 4.2 Sibling sweep on successful add: rewrite same-volume proposals
      of `new` rows to library-kind match proposals in-transaction
      (FRG-SRC-008)
- [ ] 4.3 Tests: add-degrades-to-match (no 400), sibling re-resolution,
      matched/ignored rows untouched, add-endpoint API test (first ever)
      (FRG-SRC-008)

## 5. Retry + health (FRG-SRC-009)

- [ ] 5.1 `POST /sources/entitlements/{id}/retry-download`: failed-only
      (409 otherwise), clears error, re-queues via `_queue_grab`
      (FRG-SRC-009)
- [ ] 5.2 Health: failed-source-download producer beside
      `_sources_component`, aggregated per source, remediation hint
      (FRG-SRC-009)
- [ ] 5.3 Frontend: Retry button on failed entitlement rows + hook;
      screen test (FRG-SRC-009)
- [ ] 5.4 Tests: retry happy path, non-failed 409, health degrade/clear
      (FRG-SRC-009)

## 6. Docs, gate, release

- [ ] 6.1 Manual updates: sources section (retry, health warning,
      add-degrade), import section (Humble idioms resolve)
      (FRG-PROC-011)
- [ ] 6.2 Traceability matrix regeneration; soup_check green (no dep
