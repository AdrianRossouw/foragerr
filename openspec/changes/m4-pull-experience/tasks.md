...
- [ ] 1.1 Extend `pull/commands.py::_fetch_weeks()` to `[previous, current,
      next]`; ensure an empty future-week payload is a logged skip, not an
      outage (FRG-PULL-009)
- [ ] 1.2 Tagged pytest coverage in `backend/tests/pull/test_pull_commands.py`:
...
- [ ] 1.3 Projection/API regression: `GET /api/v1/pull?week=<next>` returns
...
- [ ] 3.2 Per-entry actions on linked cards: want/skip via
...
- [ ] 3.3 "New this week" strip from `matchType === 'new_series'` rows
      (excluded from the agenda), add affordance via `navigate('/add',
      {state: {prefillTerm}})`; strip absent when none (FRG-PULL-008)
- [ ] 3.4 `CalendarScreen.test.tsx` vitest coverage, requirement IDs in test
...
- [ ] 4.1 Manual: `docs/manual/user/web-ui.md` new "Weekly pull / Calendar"
      section; README screenshot set gains calendar via
      `tools/refresh-readme-shots.sh` (FRG-UI-018, FRG-PROC-011/017)
- [ ] 4.2 Registry flips FRG-UI-018 / FRG-PULL-007..009 → implemented;
