...
- [x] 1.1 Create branch `process/roadmap-single-source`; allocate FRG-PROC-018
...
- [x] 2.1 Write `docs/roadmap.md`: entries for remaining M4 work (pull screen,
      ...
      FRG-PROC-014)
- [x] 2.2 Shrink `README.md` Roadmap section to a pointer at `docs/roadmap.md`
      (heading retained), removing the stale M4 design-refresh entry
      (FRG-PROC-014, FRG-PROC-018)
- [x] 2.3 Sweep `docs/manual/**` for forward references (start:
...
- [x] 3.1 Add `backend/tests/test_roadmap_consistency.py` tagged
...
- [x] 3.2 Same module: freshness check — parse `FRG-*` ids from
      `docs/roadmap.md`, fail on `implemented` status or unknown id in the
      requirements registry (FRG-PROC-018)
- [x] 3.3 Retarget `backend/tests/test_public_labelling.py` roadmap assertions:
