...
- [x] 1.1 `DEFAULT_NON_COMIC_PUBLISHERS` constant (shape + `*`-substring
      semantics mirroring `DEFAULT_IGNORED_PUBLISHERS`); one source of truth
- [x] 1.2 Library-wide config value seeded from it (fresh install only);
      `sources/classify.py` reads the library-wide list, not per-source
- [x] 1.3 Config resource (get/set) for the list, replacing the per-source
      publisher-rules PATCH on `api/sources.py`
- [x] 1.4 Migration: union existing per-source `publisher_rules` into the
      library-wide value (deduped, decrypt-failure contributes nothing)
- [x] 1.5 Tests: defaults present on fresh install + removable; existing
...
- [x] 2.1 Remove `PublisherRules` from `StoreManage`; add a
      publisher-filtering panel to Settings beside the CV ignore list
- [x] 2.2 Plain copy (RPG / tech-book / art-book examples; no "escape
      hatch," no single-genre framing); next-sync + sticky-decision note
- [x] 2.3 Tests (vitest): panel in Settings shows defaults + add/remove;
