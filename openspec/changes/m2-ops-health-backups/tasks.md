...
- [x] 1.1 Extract a shared `write_consistent_backup(db_path, dest_dir)` from
...
- [x] 1.2 `BackupDatabaseCommand` (`exclusivity_group="backup"`) + handler:
...
- [x] 1.3 Startup `PRAGMA quick_check` hook: failure logs loudly and marks the
      ...
      [FRG-DB-012]
- [x] 1.4 Startup restore-marker hook (`/config/restore-from`): resolve target
      ...
      no-op. [FRG-DB-010]
- [x] 1.5 Health-aggregation service (`health/service.py`): compute the
...
- [x] 2.1 Register the `backup-database` scheduled task in `app.py`
...
- [x] 2.2 Extend `api/system.py` `system_status`: add runtime (uptime, python,
      ...
      leak. [FRG-API-014]
- [x] 2.3 `GET /api/v1/health` (warnings list from the health service:
...
- [x] 2.4 `GET /api/v1/system/health` (per-component view from the service).
      Tests: every component with state + timestamps; recovery reflected on
      re-request. [FRG-NFR-011]
- [x] 2.5 `GET /api/v1/system/task` (enriched `scheduler.status()` with command
...
- [x] 3.1 System nav group + Status screen: version/build, managed paths,
      runtime table; no secret rendered. Vitest per delta scenario. [FRG-UI-016]
- [x] 3.2 Health screen: warnings list with type icons + remediation hints, and
...
- [x] 3.3 Tasks screen: scheduled-task table (interval, last/next run) with
...
- [x] 4.1 Security: add accepted-risk row **RISK-041** (plaintext provider
      ...
      [FRG-PROC-006]
- [x] 4.2 Manual: `admin/configuration.md` (backup interval + retention +
