...
- [ ] 1.1 Replace `COVER_HOSTS` frozenset with allowlist rules (host,
...
- [ ] 1.2 Tagged abuse tests (`@pytest.mark.req("FRG-META-021")`):
...
- [ ] 2.1 Migration 0029: nullable `cover_url`, `description`, `upc`,
      `creators`, `characters` on `pull_entries`; ORM `PullEntryRow` +
      `ParsedPullEntry` fields
- [ ] 2.2 `_parse_entry` reads `covers`/`description`/`creators`/
...
- [ ] 2.3 Tagged tests: idempotent enrichment storage w/ volatile
...
- [ ] 4.3 CalendarScreen covers: lazy thumbnails via
...
- [ ] 4.4 Tagged vitest suites (IDs in test names): FRG-PULL-008
