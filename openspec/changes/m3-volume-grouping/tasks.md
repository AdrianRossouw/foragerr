...
- [ ] A.1 `series_groups` table (id, title, grouping_key UNIQUE, manual_title bool,
      ...
      [FRG-SER-016]
- [ ] A.2 `library/grouping.py::franchise_key(title)` — strip a trailing `(YYYY)` and
      ...
      [FRG-SER-016]
- [ ] A.3 Auto-group at add + refresh: find-or-create the group for `franchise_key` and
...
- [ ] A.4 Override (edit flow, `aliases`-precedent): reassign (set group + lock),
...
- [ ] A.5 **Non-regression proof**: a test asserting `wanted_issues()` and
...
- [ ] A.6 `GET /api/v1/series/groups` — a SINGLE aggregate query (join groups←series←
...
- [ ] B.1 `useSeriesGroups()` hook over `GET /series/groups`; `SeriesResource` type gains
      `series_group_id`. [FRG-UI-021]
- [ ] B.2 Grouped display mode on the Comics screen (an orthogonal "group by franchise"
...
- [ ] B.3 Rename/reassign affordance reachable from a group header (calls the edit
      flow); vitest for the interaction. [FRG-UI-021, FRG-SER-017]

## C. Docs, traceability, gate

- [ ] C.1 Manual (FRG-PROC-011): `docs/manual/user/` library/browsing section documents
      the grouped view + correcting a group; README labelling if it enumerates views.
      No security/SOUP change. [FRG-PROC-011]
- [ ] C.2 Registry + matrix: FRG-SER-016/017, FRG-API-020, FRG-UI-021 flip
