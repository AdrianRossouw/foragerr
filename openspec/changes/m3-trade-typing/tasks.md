...
- [ ] A.1 `SeriesRow.booktype` (nullable Text) + `booktype_locked` (bool default false);
...
- [ ] A.2 `library/booktype.py::detect_series_booktype(title)` reusing `parser.vocab`
...
- [ ] A.3 Auto-derive at add + refresh when NOT `booktype_locked`; operator override via
...
- [ ] A.4 **Non-suppression invariant (the core)** — do NOT touch `wanted_issues`/
...
- [ ] A.5 `booktype` on `SeriesResource` (via `_series_fields`); accepted on `SeriesEdit`
...
- [ ] B.1 `SeriesResource` type gains `booktype`; a small book-type badge (TPB/GN/HC) on
      ...
      [FRG-UI-022]
- [ ] B.2 A library filter toggle (all / collected / single-issues) in the toolbar/view
...
- [ ] C.1 Manual (FRG-PROC-011): `docs/manual/user/library.md` documents collected-edition
...
- [ ] C.2 Registry + matrix: FRG-SER-018/019, FRG-UI-022 flip `proposed → implemented`;
