...
- [x] 1.1 Config: `server.trusted_proxies` (list, default empty) with
      FRG-DEP-004 versioned config migration; env override per config
      conventions
- [x] 1.2 Perimeter resolution: resolve effective scheme + client IP once
...
- [x] 4.2 STRIDE/threat-model + risk-register updates: trusted-proxy
...
- [x] 4.3 Manual deployment-security section (TLS stories, FDE, container
      run flags, trusted proxy + warning, downgrade note); README
      consistency check
- [x] 4.4 `docs/roadmap.md`: M9 marked complete; M10 in progress (posture
      first, release-pipeline second)

## 5. Gate and release

- [x] 5.1 Registry rows → `implemented`; regenerate traceability matrix;
