...
- [x] 1.1 Migration 0032: `source_entitlements.duplicate_of` (nullable) +
      `(source_id, md5)` index; forward-only
- [x] 1.2 Sync-time linking (same source, same stored md5, `new` rows
      only; canonical = earliest; insert path parks newcomers) — decided
      rows never re-linked
- [x] 1.3 Startup backfill, one-shot/idempotent: link existing sets only
      when every member is still `new`
- [x] 1.4 `duplicate` review state wired through counts, default views,
...
- [x] 1.5 Resources: canonical row exposes copy count + copy bundle
      identities; rows expose `review_status = duplicate`
- [x] 1.6 Tests (tagged FRG-SRC-015 / FRG-SRC-004): link-at-sync,
...
- [x] 2.1 Candidate resources (lookup + suggest) gain the derived
      collected-cues flag from the shared booktype vocabulary; no
      ranking change
- [x] 2.2 `EntitlementSearch`: pasted URL/id routes to the FRG-API-026
      volume-id hook, renders the resolved volume as a pickable
      candidate; unknown id shows the not-found note
- [x] 2.3 Badge rendering (reuse BookTypeBadge or equivalent, soft
      wording); group-header picker inherits (no fork)
- [x] 2.4 Tests: backend flag derivation (synthetic titles); vitest for
...
- [x] 3.1 Server: display-group merge of containment-related stripped
...
- [x] 3.2 Server: parsed (volume_ordinal, issue_number) sort key on the
      entitlement resource (parser derivation, single fold)
- [x] 3.3 Frontend: sort group members by the server key (unknowns
      last, then name); merged-group labeling handles contained-key
      names (no dangling prefix derivation)
- [x] 3.4 Duplicates filter + copies chip + dimmed Restore presentation
      (FRG-UI-029 scenarios)
- [x] 3.5 Tests: fold-merge cases (synthetic two-name franchise), sort
      order incl. unknowns, sweeps still exact-fold (regression), vitest
      list presentation

## 4. Docs + verification

- [x] 4.1 `docs/manual/user/sources.md` review section: duplicates,
      filter, group order, picker id paste + cue
- [x] 4.2 Registry statuses + trace regen; full backend (xdist) +
