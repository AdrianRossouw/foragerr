...
- [ ] 1.1 Add `person_credits` to `ISSUE_FIELDS`; extend `IssueRecord` +
...
- [ ] 3.1 `creators-backfill` command: dedup fan-out over `refresh-series`,
...
- [ ] 3.2 `api/creators.py`: paged list with aggregates + bounded work refs,
...
- [ ] 4.1 RISK-011 ingest-arm note (credits strings through the sanitizer);
      registry flips to implemented; matrix regen; soup_check green
      (FRG-PROC-002/005/006)
- [ ] 4.2 CHANGELOG + version bump on-branch (v0.5.0 — M5 opens); full
