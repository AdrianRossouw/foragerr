...
- [ ] 1.2 docs/security updates: risk-register entry for the GitHub Actions
...
- [ ] 2.1 `site/build.py` skeleton: artifact loaders for registry, matrix,
      ...
      [FRG-SITE-001]
- [ ] 2.2 Derived data: requirement/release/milestone counts, traced-test coverage
...
- [ ] 2.3 Templates for the five pages ported from the design (hero A + trace
      ...
      `site/static/site.css` token port; vendored Roboto Mono + README screenshots
      copied at build [FRG-SITE-002]
- [ ] 2.4 Truthfulness scans in the build: banned-phrase list
...
- [ ] 3.1 Generator unit tests against fixture artifacts: fact-tracking and
...
- [ ] 3.2 Workflow lint test: parse `.github/workflows/pages.yml`, assert SHA-pinned
      `uses:` and exact least-privilege permissions
      [`@pytest.mark.req("FRG-SITE-005")`]

## 4. Deployment & docs

- [ ] 4.1 `.github/workflows/pages.yml` (push to main + workflow_dispatch; build →
      upload-pages-artifact → deploy-pages; SHA-pinned; `contents: read`,
      `pages: write`, `id-token: write`) [FRG-SITE-005]
- [ ] 4.2 README: add site link under the header block; verify no manual pages are
      affected (manual impact: none — declared in proposal) [FRG-SITE-006]
- [ ] 4.3 Local end-to-end verify: build site from the real repo, open output,
