...
1. **Backend: `rarfile` (ISC) + `unrar-free` (GPL, Debian `main`), subprocess boundary.** Feasibility probe (2026-07-13) verified `unrar-free` extracts both RAR4 and RAR5 correctly inside the actual `python:3.12-slim` runtime image, and Debian's package registers `/usr/bin/unrar` via update-alternatives — `rarfile` detects it with no code-side override. The whole stack is OSI-licensed with no `non-free` apt component. RARLAB's proprietary `unrar` remains the documented compatibility alternative if the corpus run surfaces archives `unrar-free` mishandles; `bsdtar`/libarchive is a second fallback `rarfile` supports natively. The subprocess boundary means no license contamination of foragerr code and no in-process parsing of untrusted RAR structures. The 366-file corpus is the acceptance bar for backend claims.
