...
- [x] 1.1 POST /queue/remove {ids, blocklist, deleteData} looping the
...
- [x] 1.2 Tests: mixed batch (one importing → per-row error, rest
      removed), blocklist rows written, client-removal failure never
      blocks de-tracking; tags FRG-DL-008

## 2. Queue screen (FRG-UI-006)

- [x] 2.1 Compose components/dataTable.module.css; tableWrap scroll
      container; wrap release tokens; fix the flex-on-td actions cell;
      no page-level horizontal scroll
- [x] 2.2 Failed rows: no progress bar/byte counts; status+reason carry
      it; series/issue still identify hash-titled rows
- [x] 2.3 Selection column + select-all (Blocklist pattern), bulk Remove
      via the existing dialog, Clear failed (select+confirm+bulk),
      PageControls paging
- [x] 2.4 Vitest: overflow containment (class/structure), failed-row
...
- [x] 3.1 docs/manual/user/web-ui.md queue section
- [x] 3.2 Suites green (backend xdist, frontend serialized), tools exit
