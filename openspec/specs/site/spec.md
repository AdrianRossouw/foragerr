...
The public project site (`adrianrossouw.github.io/foragerr`) that presents foragerr's
regulated-software-development story. Every displayed fact is generated at build time
from committed repository artifacts — the requirements registry, traceability matrix,
CHANGELOG, risk register, LICENSE, and git tags — so the site is a truthful, always-
current account of how the project is made rather than a hand-maintained marketing
page. It is deployed by the repository's GitHub Actions workflow on every push to
`main`.
