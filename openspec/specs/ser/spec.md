...
- **Notes**: Mylar's Active/Paused/Loading series status collapses into `monitored` (bool) plus transient command state — deliberate divergence (D1). ComicVine ID conventions (4050- volume prefix) are normalized at the META boundary, not stored in SER keys.
