...
export function SeriesDetail() {
  ...
  return (
    <>
      ...
      {/*
       * Interactive-search mount seam: the FRG-UI-007 overlay replaces this
       * stub's body; the launch contract (uiStore.interactiveSearchIssueId)
       * is the stable part.
       */}
      {interactiveIssueId !== null && (
        <div className={styles.overlay}>
          <div
            role="dialog"
            aria-modal="true"
            aria-label={`Interactive search for issue ${interactiveIssueId}`}
            className={styles.searchOverlay}
            data-testid="interactive-search-overlay"
            data-issue-id={interactiveIssueId}
          >
            <header className={styles.dialogHeader}>
              <strong>Interactive Search — issue {interactiveIssueId}</strong>
              <button
                type="button"
                className={styles.iconButton}
                aria-label="Close"
                onClick={closeInteractiveSearch}
              >
                <CloseIcon size={14} />
              </button>
            </header>
            <div className={styles.dialogBody}>
              <p>Release decisions render here (FRG-UI-007 overlay).</p>
            </div>
          </div>
        </div>
      )}
